...
// Package http provides an HTTP client for the Authentication service.
package http
...
func New(instance string, options ...httptransport.ClientOption) (pb.AuthenticationServer, error) {
	...
	var SignOutZeroEndpoint endpoint.Endpoint
	{
		SignOutZeroEndpoint = httptransport.NewClient(
			"GET",
			copyURL(u, "/auth/logout"),
			EncodeHTTPSignOutZeroRequest,
			DecodeHTTPSignOutResponse,
			options...,
		).Endpoint()
	}
...
// DecodeHTTPSignOutResponse is a transport/http.DecodeResponseFunc that decodes
// a JSON-encoded SignOutResponse response from the HTTP response body.
// If the response has a non-200 status code, we will interpret that as an
// error and attempt to decode the specific error message from the response
// body. Primarily useful in a client.
func DecodeHTTPSignOutResponse(_ context.Context, r *http.Response) (interface{}, error) {
	defer r.Body.Close()
	buf, err := ioutil.ReadAll(r.Body)
	if err == io.EOF {
		return nil, errors.New("response http body empty")
...
func EncodeHTTPSignInZeroRequest(_ context.Context, r *http.Request, request interface{}) error {
	...
	toRet := request.(*pb.SignInRequest)
...
// EncodeHTTPSignOutZeroRequest is a transport/http.EncodeRequestFunc
// that encodes a signout request into the various portions of
// the http request (path, query, and body).
func EncodeHTTPSignOutZeroRequest(_ context.Context, r *http.Request, request interface{}) error {
	strval := ""
	_ = strval
